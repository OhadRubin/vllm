...
def handle_opt(options):
    ...
    options = [x.split(",", 2) for x in options]
...
def get_all_experiments(experiment, config, exp_count):
    ...
    for task in experiment.tasks:
        orig_task = deepcopy(task.params)
        state = task.params
        state["WANDB_NAME"] = []
        ARG_VARS = {}
...
def set_environment_variables(config,dag,node_id,wandb_name,*node_ids):
    ARG_mapping, val_mapping, wandb_args = parse_config(config)
    ARG_VARS = {}
    if len(node_ids)==0:
        node_ids = list(range(1,100))
    mapping = {n:i for i,n in enumerate(node_ids)}
    state = dag.tasks[mapping[node_id]].params
        
    state["WANDB_NAME"] = wandb_name
    wandb_args.extend(list(state.keys()))
    val_mapping_items = list(val_mapping.items())
    for param_name, default_value in val_mapping_items[::-1]:
        if param_name in state:
    ...
    ARG_mapping_inverse = {v:k for k,v in ARG_mapping.items()}
    # Create a temporary file named {WANDB_NAME}.sh
    with tempfile.NamedTemporaryFile(mode='w', suffix='.sh', delete=False) as temp_file:
        temp_path = temp_file.name
        # Write the environment variables to the temporary file
        # temp_file.write(f"declare -A HP_DICT\n")
        for var_name, var_value in ARG_VARS.items():
            temp_file.write(f'export {var_name}={var_value}\n')
            # if var_name in ARG_mapping_inverse:
                # temp_file.write(f'HP_DICT["{ARG_mapping_inverse[var_name]}"]={var_value}\n')
